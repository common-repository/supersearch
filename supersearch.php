...
{
    $active_tab = supersearch_add_tabs();
    ...
    ?>
    <script>
        const searchitem_progress = <?php echo esc_attr($stats->data->search_item_percent ?? 0);?>;
        const searchlimit_progress = <?php echo esc_attr($stats->data->search_percent ?? 0);?>;
    </script>
    
    <div style="content">
        <?php if ($active_tab == 'usage') { ?>
            <img src="<?php echo esc_attr(plugins_url( 'supersearch_logo.png', __FILE__ ));?>" style="float:right;margin-left:20px;">
